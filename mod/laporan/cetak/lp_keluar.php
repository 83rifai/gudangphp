...
if($tgl1 == "" ){
	if($tgl2 == "" ){
		$baca = mysql_query("SELECT a.id AS kode,a.*,b.*,c.*,d.* FROM tr_brg_keluar a 
							...
							ORDER BY a.id,a.tgl_keluar");
	}else{
		$baca = mysql_query("SELECT a.id AS kode,a.*,b.*,c.*,d.* FROM tr_brg_keluar a 
							INNER JOIN td_brg_keluar b ON a.id=b.id
							INNER JOIN barang c ON b.kd_barang=c.id
							INNER JOIN pelanggan d ON a.id_pelanggan=d.id
							WHERE a.tgl_keluar <= '$tgl2'
							ORDER BY a.id,a.tgl_keluar");
	}	
}else{
	if($tgl2 <> ""){
		$baca = mysql_query("SELECT a.id AS kode,a.*,b.*,c.*,d.* FROM tr_brg_keluar a 
							...
							INNER JOIN barang c ON b.kd_barang=c.id
							INNER JOIN pelanggan d ON a.id_pelanggan=d.id
							WHERE a.tgl_keluar >= '$tgl1' AND a.tgl_keluar <= '$tgl2'
							ORDER BY a.id,a.tgl_keluar");
	}else{
		$baca = mysql_query("SELECT a.id AS kode,a.*,b.*,c.*,d.* FROM tr_brg_keluar a 
							INNER JOIN td_brg_keluar b ON a.id=b.id
...
$html = "<table style=\"border-collapse:collapse; font-size:14;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"2\">
		...
		<table style=\"border-collapse:collapse; font-size:14;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"2\">
				<tr>
					<td  align=\"center\" width=\"100%\" colspan=\"6\" style=\"font-size:24px\"><b>Laporan Barang Keluar <br>$tgl</b></td>
				</tr><br><br>
		</table> 
		<table style=\"border-collapse:collapse; font-size:14;\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"1\" cellpadding=\"2\">
				<tr>
					...
					<td  align=\"center\" width=\"20%\" bgcolor=\"#e6e6e6\"><b>Nama Pelanggan</b></td>
...
$html .= "
				<tr>
					...
					<td style=\" valign:center\" align=\"left\" width=\"20%\" bgcolor=\"$warna\">$nm_pelanggan</td>
