...
trans_produk_keluar_detail.id as id,
...
trans_produk_keluar_detail.jumlah as jumlah,
trans_produk_keluar_header.nomor_transaksi as nomor_transaksi,
trans_produk_keluar_header.tanggal,
trans_produk_keluar_header.master_pelanggan_id,
trans_produk_keluar_header.purchase_order,
trans_produk_keluar_header.delivery_order,
trans_produk_keluar_detail.konversi_satuan_id,
master_pelanggan.nama as pelanggan
FROM
...
INNER JOIN trans_produk_keluar_header ON trans_produk_keluar_header.id = trans_produk_keluar_detail.trans_produk_keluar_id
LEFT JOIN master_pelanggan ON master_pelanggan.id = trans_produk_keluar_header.master_pelanggan_id ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`127.0.0.1` SQL SECURITY DEFINER  VIEW `data_produk_masuk` AS SELECT
	trans_produk_masuk_detail.id as id,
