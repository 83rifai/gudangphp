...
elseif($mod=="kecil" AND $act=="insert"){ // this action for insert data to database satuan_kecil 
	$id				= $_POST[id];
	$id2			= $_POST[id2];
	$aksi     		= $_POST[aksi];
	$nama   		= $_POST[nama];
	...
	if($aksi <> "editkecil"){
		...
		$insert = mysql_query("insert into satuan_kecil(nama,jumlah)						
							   VALUES('$nama','$jumlah')");
...
elseif($mod=="kecil" AND $act=="hapus_kecil"){
	$delete=mysql_query("Delete from satuan_kecil where id='$_GET[id]'");
     if($delete){
                echo "<script type=\"text/javascript\">
				      alert('Data berhasil tersimpan..');
                      window.location = \" media.php?mod=$mod \";
...
elseif($mod=="sedang" AND $act=="insert"){
	...
	if($aksi <> "editsedang"){
		$insert = mysql_query("insert into satuan_sedang(nama,jumlah)						
							   VALUES('$nama','$jumlah')");
...
elseif($mod=="sedang" AND $act=="hapus_sedang"){
	$delete=mysql_query("Delete from satuan_sedang where id='$_GET[id]'");
     if($delete){
                echo "<script type=\"text/javascript\">
				      alert('Data berhasil tersimpan..');
                      window.location = \" media.php?mod=$mod \";
...
elseif($mod=="besar" AND $act=="insert"){
	...
	if($aksi <> "editbesar"){
		$insert = mysql_query("insert into satuan_besar(nama,jumlah)						
							   VALUES('$nama','$jumlah')");
		...
		$update=mysql_query("Update satuan_besar set nama ='$nama',
												jumlah ='$jumlah'
												where id='$id2'");
...
elseif($mod=="besar" AND $act=="hapus_besar"){
	$delete=mysql_query("Delete from satuan_besar where id='$_GET[id]'");
     if($delete){
                echo "<script type=\"text/javascript\">
				      alert('Data berhasil tersimpan..');
                      window.location = \" media.php?mod=$mod \";
